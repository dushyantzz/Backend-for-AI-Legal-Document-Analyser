  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setChats((prevChats) => {
        ...
        updatedChats[updatedChats.length - 1].ai_chat = response.response; 
        return updatedChats;
      });
    } catch (err) {
      console.log(err);
      setChats((prevChats) => {
        const updatedChats = [...prevChats];
        updatedChats[updatedChats.length - 1].ai_chat = "Sorry, I'm having trouble processing your request. Please try again."; 
        return updatedChats;
      });
    }
  };
